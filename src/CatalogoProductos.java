...
// Aqui la clase que representa un catálogo de productos
public class CatalogoProductos {
    // Aca la lista que almacena los productos del catálogo
    ...
    public List<Producto> getProductosPorCategoria(String categoria) {
        ...
        // Aca se recorre la lista de productos y se comparan las categorías (ignorando mayúsculas/minúsculas)
        for (Producto p : productos) {
            if (p.getCategoria().equalsIgnoreCase(categoria)) {
                recomendados.add(p); // Aqui si la categoría coincide, se agrega a la lista de recomendados
            }
        }
        return recomendados; // Aca se retorna la lista de productos filtrados por categoría
    }

    // El metodo que retorna todos los productos del catálogo
